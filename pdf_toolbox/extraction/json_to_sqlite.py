...
def _split_elements_json(src):
    ...
    for dictionary in processing:
        ...
        for item in split_path:
            if split_path[-1] in utils.constants.figures:
                temp_dict["Element Type"] = split_path[-1]
                final.append(temp_dict)
                break
            if item in utils.constants.headings:
                temp_dict["Element Type"] = item
                final.append(temp_dict)
                break
            if item in utils.constants.lists:
                temp_dict["Element Type"] = item
                final.append(temp_dict)
                break
            if item in utils.constants.paragraphs:
                temp_dict["Element Type"] = item
                final.append(temp_dict)
                break
            if item in utils.constants.table_rows:
                temp_dict["Element Type"] = item
                final.append(temp_dict)
                break
    ...
    for sub_dict in json_file:
        ...
        for key, val in sub_dict.items():
            if key == "filePaths":
                ...
                if sub_dict[key] in utils.constants.unwanted_pdf[::]:
                    break
                ...
                processing.append(temp_dict)
                txt_out = utils.constants.confidence_dir / src.parent.name / f"{src.parent.name}-JSON-TXT.txt"
